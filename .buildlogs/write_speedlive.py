#!/usr/bin/env python3
"""Atomic writer for the refactored SpeedLiveActivityView.swift.

Existing write_file / bash-heredoc paths kept mangling the Swift
string-interpolation quotes, so this script emits the file in one shot
with Python triple-quotes — no bash involved.
"""
import os

PATH = "SmartSpeedCompanion/Features/iOS26/LiveActivities/SpeedLiveActivityView.swift"

# Original speed-phrase interpolation, used in many Text() calls.
SPEED_TEXT_LINE = 'Text("\\(Int(context.state.speed))")'

# Conditional pill-dash text for the speed-limit line.
LIMIT_DASH_TEXT = '"LIMIT \\u{2014}"'

CONTENT = f"""// Path: Features/iOS26/LiveActivities/SpeedLiveActivityView.swift
//
...
with open(tmp, "w", encoding="utf-8", newline="\n") as f:
    f.write(CONTENT)
os.replace(tmp, PATH)
print(f"wrote {PATH}: {os.path.getsize(PATH)} bytes, {CONTENT.count(chr(10))} newlines")
